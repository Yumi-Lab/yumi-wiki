...
# Env (optionnels) :
#   LOOP_SCRIPT=loop.sh     nom du script de boucle, pour pgrep (ex. run-v7.sh, run-vN.sh…)
...
last=0; prev_head="$(head_sha)"; stale=0
while true; do
  ...
  if ! pgrep -f "$LOOP_SCRIPT" >/dev/null 2>&1; then
    echo "⏹ $(date '+%H:%M') BOUCLE ARRÊTÉE (fin/crash/max_cycles) → inspecte $MONITOR_DIR (logs + .err + verdict)."; exit 0
  fi
