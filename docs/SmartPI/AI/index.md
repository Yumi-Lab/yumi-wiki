...
The Smart Pi One and Smart Pad are 32-bit ARM (Allwinner H3 / armv7l) single-board computers. Most modern AI command-line tools ship only for 64-bit machines and refuse to install on this architecture. YUMI-LAB maintains dedicated installers that bring six of today's leading AI coding assistants to the board — running an interactive AI agent directly in the terminal, on hardware with as little as 1 GB of RAM.
...
[![Codex CLI on Smart Pi One](/img/SmartPi/AI/codex-cli-banner.svg)](SmartPi_AI_Codex_CLI.md)
...
**Yumi Code CLI** is our own agent, written in Go for these boards — announced here when it ships. The six above are available today.
...
| ![](/img/SmartPi/AI/codex-logo.svg){ .off-glb width="18" } **[Codex CLI](SmartPi_AI_Codex_CLI.md)** | **Native when available** — cross-compiled armv7, falls back to QEMU emulation otherwise | OpenAI account (device-auth) or API key | `CODEX_CPUS` (+ `CODEX_ENGINE`) | [codex-cli-smartpi](https://github.com/Yumi-Lab/codex-cli-smartpi){ target=_blank } |
...
All six can sign in with your existing **provider account** — no API key, no separate developer billing. **Codex CLI**, **Kimi Code CLI**, **Kimi CLI** and **Vibe CLI** additionally accept a direct API key if you prefer. (Kimi CLI and Kimi Code CLI are two separate Moonshot tools — see [which to choose](#6-which-one-should-i-choose).)

**Always the latest version.** Every project installs the **newest published release** of its CLI, natively where the platform allows. Grok always runs under emulation — its official binary is 64-bit only. Codex CLI runs natively whenever a native armv7 build exists for the resolved version, and falls back to the same emulation path otherwise — see [Getting the native build](SmartPi_AI_Codex_CLI.md#2-getting-the-native-build). The installer **is also the updater** — re-run it to move to the latest. Each repo ships a `<cli>-check-update` probe that prints `{installed, latest, update_available}`, and you can pin a specific version if you ever need to.

### Controlling CPU usage

Every CLI runs through a `taskset` / `nice` wrapper, so you can bound how much of the board it takes. There are two knobs:

- **`<CLI>_CPUS`** — cores used by the **running agent**: `CLAUDE_CPUS`, `CODEX_CPUS`, `GROK_CPUS`, `KIMI_CPUS` (`KIMI_CODE_CPUS`), `VIBE_CPUS`.
...
- Access to the matching provider: a **Claude Pro/Max** account (Claude Code), an **OpenAI account or API key** (Codex CLI), an **X (x.com) or grok.com / SuperGrok** account (Grok CLI), a **Kimi account or API key** (Kimi Code CLI / Kimi CLI), or a **Mistral account or API key** (Vibe CLI)
...
All six give you an interactive terminal UI and one-shot (`-p "question"`) query mode. Pick the one that matches the account or key you already have:
...
- Choose **[Codex CLI](SmartPi_AI_Codex_CLI.md)** if you have an OpenAI account (or API key). It runs natively whenever a native armv7 build is available, falling back to emulation otherwise — same commands either way.
